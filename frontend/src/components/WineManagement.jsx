...
const WineTypeDistribution = ({ wines = [] }) => {
  ...
  const typeCount = wineTypes.reduce((acc, type) => {
    acc[type] = wines.filter(
      (wine) =>
        wine.wine_type?.toLowerCase() === type.toLowerCase() ||
        (!wine.wine_type && type === "Altres")
    ).length;
    return acc;
  }, {});
      ...
      <div className="relative">
        ...
        <div
          ...
          style={{
            backgroundColor: getWineTypeColor(wine.wine_type?.toLowerCase()),
            color:
              wine.wine_type?.toLowerCase() === "blanc" ||
              wine.wine_type?.toLowerCase() === "espumós"
                ? "#333"
                : "white",
          }}
        >
...
export default function WineManagement() {
  ...
  const filteredWines =
    activeFilter === "all"
      ? wines
      : wines.filter((wine) => wine.status === activeFilter);
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-sm text-gray-500">
                  ...
                  {filteredWines.length === 1 ? "vi" : "vins"}
                </span>
              </div>
