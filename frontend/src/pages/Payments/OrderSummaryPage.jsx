...
export default function OrderSummaryPage() {
  ...
  const shippingCost =
    Number.parseFloat(localStorage.getItem("shippingCost")) || 0;
  const platformFees =
    Number.parseFloat(localStorage.getItem("platformFees")) || 0;
  ...
  const formatDate = (dateString) => {
    ...
    return date
      .toLocaleDateString("ca-ES", options)
  ...
  const markOrderAsCompleted = async (orderId) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "Failed to mark order as completed",
        );
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchOrderDetails = async () => {
      ...
      try {
        ...
        const allOrders = Object.entries(ordersByOrderId).map(
          ([orderId, items]) => {
            return {
              id: orderId,
              ...
              items: items.map((item) => ({
                ...
                price: Number.parseFloat(item.price_restaurant) || 0,
        ...
        // Solo se marcan como completados los pedidos que de verdad formaban
        // parte de este pago (Stripe ha vuelto a redirigirnos con
        // redirect_status=succeeded), y solo esos IDs concretos — nunca
        // "todos los pedidos pendientes que tenga el usuario ahora mismo".
        // La comprobación de que el pago existe y está en 'succeeded' la
        // hace el backend (OrderController::completed) contra Stripe; esto
        // es solo para no llamar al endpoint para pedidos que no vienen al
        // caso.
        if (isPaymentSuccessful) {
          let paidOrderIds = [];
          try {
            paidOrderIds = JSON.parse(
              localStorage.getItem("currentOrderIds") || "[]",
            ).map(String);
          } catch {
            paidOrderIds = [];
          }
  ...
  // Les despeses d'enviament s'apliquen un únic cop per a tot el pagament
  // (es calculen a CartSummary.jsx quan es fa "Finalitzar compra"), no per
  // cada comanda/restaurant per separat. Aquí només calculem el subtotal
  // real de cada targeta a partir del preu de cada article.
      ...
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-xl shadow-lg p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            No s'ha pogut verificar la seva identitat. Si us plau, iniciï sessió
            i torni a intentar-ho.
          </p>
          <button
            onClick={() => navigate("/")}
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-xl font-bold text-center text-gray-900 mb-1">
            GRÀCIES PER LES TEVES COMANDES!
          </h1>
          <p className="text-sm text-gray-500 text-center">
            Les confirmacions de les comandes han estat enviades a{" "}
        ...
        {/* Resum global del pagament — les despeses d'enviament només es
            cobren un cop per a tot el pagament, no per cada comanda */}
        {orderDetails && orderDetails.length > 0 && (
          <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 mb-6">
            <div className="p-6">
              ...
              <div className="flex justify-between text-sm py-1">
                <span className="text-gray-600">Comissions i taxes</span>
